  ...
  "relative w-full rounded-lg border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5",
  {
    variants: {
      variant: {
        ...
        // This variant is now fully corrected
        destructive:
          "border-red-600 text-red-600 [&>svg]:text-red-600 [&_[data-slot=alert-description]]:text-red-600",
      },
    },
    defaultVariants: {
      variant: "default",
...
function AlertDescription({
  ...
  return (
    <div
      ...
      // Changed back to a neutral default color
      className={cn(
